# Create worker.js with proper JavaScript
$workerJs = @'
...
app.use('*', cors({
  origin: (origin) => {
    const allowed = ['http://localhost:3000', 'http://localhost:5173', 'https://upalit.pages.dev'];
...
app.get('/api/admin/users', authenticate, requireAdmin, (c) => {
  ...
  if (q) {
    ...
    users = users.filter(u => u.email.toLowerCase().includes(search) || u.fullName.toLowerCase().includes(search));
  }
  users.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const start = (parseInt(page) - 1) * parseInt(limit);
  const end = start + parseInt(limit);
  const paginated = users.slice(start, end).map(({ password, ...rest }) => rest);
  return c.json({ data: paginated, total: users.length, page: parseInt(page), limit: parseInt(limit) });
});
...
app.patch('/api/admin/users/:id/role', authenticate, requireAdmin, async (c) => {
  ...
  return c.json({ message: 'Role updated', user: { id: user.id, email: user.email, role: user.role } });
});
...
app.patch('/api/admin/users/:id/disable', authenticate, requireAdmin, async (c) => {
  ...
  return c.json({ message: disabled ? 'User disabled' : 'User enabled' });
});
...
app.get('/api/admin/properties', authenticate, requireAdmin, (c) => {
  ...
  if (q) {
    const search = q.toLowerCase();
    properties = properties.filter(p => p.name.toLowerCase().includes(search) || p.location.toLowerCase().includes(search));
  }
  properties.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const start = (parseInt(page) - 1) * parseInt(limit);
  const end = start + parseInt(limit);
  const paginated = properties.slice(start, end);
  return c.json({ data: paginated, total: properties.length, page: parseInt(page), limit: parseInt(limit) });
});
...
app.patch('/api/admin/properties/:id', authenticate, requireAdmin, async (c) => {
  ...
  db.auditLog.push({
    ...
    targetPropertyId: id,
    details: { before: { name: oldValues.name, price: oldValues.price }, after: { name: property.name, price: property.price } },
    timestamp: new Date().toISOString(),
  });
...
app.get('/api/admin/audit-log', authenticate, requireAdmin, (c) => {
  ...
  return c.json({ data: enriched, total: logs.length, page: parseInt(page), limit: parseInt(limit) });
});
...
$workerJs | Out-File -FilePath src\worker.js -Encoding utf8